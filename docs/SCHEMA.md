...
- **H2 dev fallback** (`quarkus.profile=dev`): For local runs without PostgreSQL; skip RLS and use app-level `tenant_id` filter only. (STORY-022: legacy `%sqlite` alias removed — use `dev` only.)
